...
public interface IProjectRepository extends Neo4jRepository<Project, String> {
    ...
    @Query("MATCH (pr:Project)-[:HAS_TEAM]->(tm:Team) " +
            "WHERE elementId(pr) = elementId " +
            "RETURN tm.elementId")
    String getTeamElementIdByElementId(String elementId);

    @Query("MATCH (pr:Project)-[:HAS_TUTOR]->(pe:Person) " +
            "WHERE elementId(pr) = elementId " +
            "RETURN pe.elementId")
    String getTutorElementIdByElementId(String elementId);

    @Query("MATCH (pr:Project)-[:HAS_OBJECTIVE]->(ob:Objective) " +
            "WHERE elementId(pr) = elementId " +
            "RETURN ob.elementId")
    String getObjectiveElementIdByElementId(String elementId);

    @Query("MATCH (pr:Project)-[:HAS_TASK]->(ts:Task) " +
            "WHERE elementId(pr) = elementId " +
            "RETURN ts.elementId")
    String getTaskElementIdByElementId(String elementId);
